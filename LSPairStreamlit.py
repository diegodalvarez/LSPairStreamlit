...
class LSPairStreamlit:
    ...
    def _ols_slice(self, df, position: str, sample_group: str) -> pd.DataFrame:

        df_slice = (df.query(
            "position == @position & sample_group == @sample_group").
            drop(columns = ["position", "sample_group", "ticker", "benchmark_ticker"]))
        
        df_alpha = (df_slice.query(
            "parameter == 'alpha'").
            drop(columns = ["parameter"]))
        
        df_beta = (df_slice.query(
            "parameter == 'beta'").
            drop(columns = ["parameter"]))
