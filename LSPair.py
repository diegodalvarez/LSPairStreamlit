    ...
    def _rolling_ols(
        ...
        for lookback_window in lookback_windows:
            ...
            try:
                ...
                rolling_ols_params = (pd.DataFrame(
                    rolling_ols.params).
                    dropna().
                    reset_index())
                
                # not ideal saving variable to memory so that I can reference columns by index and then change
                # this needs to be fixed (another day)
                
                rolling_ols_params = (rolling_ols_params.rename(
                    columns = {
                        "const": "alpha",
                        rolling_ols_params.columns[1]: "beta",
                        "index": id_vars_name}).
        ...
        for k, position in enumerate(df_tmp.position.drop_duplicates().sort_values().to_list()):
            for j, parameter in enumerate(df_tmp.parameter.drop_duplicates().sort_values().to_list()):
                for i, sample in enumerate(["in_sample", "out_sample", "full_sample"]):
                    ...
                    display(df_plot_tmp)
                    display(df_plot_tmp.variable.drop_duplicates())
